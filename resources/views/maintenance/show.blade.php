...
@section('title', 'Work Order #T-'.($workOrder?->id ?? '2033').' - Nestora')
...
<div class="db-header">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; width: 100%;">
        <h1 class="db-title" style="font-size: 1.5rem;">Work Order #T-{{ $workOrder?->id ?? '2033' }}</h1>
...
<div class="grid grid-3" style="align-items: start;">
    <div style="grid-column: span 2; display: flex; flex-direction: column; gap: 2rem;">
        <div class="card">
            <h3 style="font-size: 1.25rem; margin-bottom: 1rem;">{{ $workOrder?->title ?? 'Bathroom pipe leakage in master washroom' }}</h3>
            ...
            <div style="display: flex; gap: 1.5rem; margin-bottom: 1.5rem; font-size: 0.85rem; border-bottom: 1px solid var(--border-color); padding-bottom: 1rem; flex-wrap: wrap;">
                <div><span class="text-muted">Target Flat:</span> <strong style="color: var(--primary-color);">{{ $flat?->flat_number ?? 'Flat 3B' }} ({{ $flat?->building?->name ?? 'Building A' }})</strong></div>
                <div><span class="text-muted">Urgency:</span> <span class="badge badge-{{ in_array($workOrder?->priority, ['high', 'urgent', 'emergency'], true) ? 'rejected' : 'pending' }}" style="font-size: 0.65rem; padding: 0.1rem 0.4rem;">{{ $workOrder?->priority ?? 'high' }}</span></div>
                <div><span class="text-muted">Assigned Date:</span> <strong style="color: var(--text-primary);">{{ $workOrder?->created_at?->format('M d, Y') ?? 'July 02, 2026' }}</strong></div>
                <div><span class="text-muted">Deadline Date:</span> <strong style="color: var(--color-rejected);">{{ $workOrder?->due_at?->format('M d, Y') ?? 'July 04, 2026' }}</strong></div>
            </div>
            ...
            <p style="font-size: 0.9rem; line-height: 1.6; color: var(--text-secondary); margin-bottom: 1.5rem;">
                {{ $workOrder?->complaint?->description ?? 'There is water leaking slowly from the joint underneath the washroom basin sink. Please check it before it damages flooring.' }}
            </p>
            ...
            <p style="font-size: 0.9rem; line-height: 1.6; color: var(--text-secondary); margin-bottom: 0; font-style: italic; background-color: var(--bg-main); padding: 1rem; border-radius: var(--radius-md);">
                {{ $workOrder?->instructions ?? 'Please bring sink replacement seals. Ring bell or contact ullas before visiting flat to confirm availability.' }}
            </p>
        </div>
        ...
        <div class="card">
            ...
            <div class="grid grid-2" style="font-size: 0.875rem;">
                <div>
                    ...
                    <div style="font-weight: 700; color: var(--text-primary); font-size: 1rem; margin-top: 0.25rem;">{{ $resident?->name ?? 'ullas' }}</div>
                </div>
                <div>
                    <span class="text-muted">Contact Phone:</span>
                    <div style="font-weight: 700; color: var(--primary-color); font-size: 1rem; margin-top: 0.25rem;">{{ $resident?->phone ?? '+880 1711 223344' }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div style="grid-column: span 1; display: flex; flex-direction: column; gap: 2rem;">
        ...
        <div class="card">
            ...
            @forelse ($workOrder?->notes ?? collect() as $note)
